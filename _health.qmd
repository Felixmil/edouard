{{< include _setup.qmd >}}

## 🏥 Santé


### Températures

```{r}
#| fig.width: 8
#| fig.asp: 0.5

library(ggiraph)

temp_data <- get_temperature_data(data)

if (nrow(temp_data) > 0) {
  colors <- edouard_colors()

  plot_temp <- temp_data |>
    mutate(
      tooltip = if_else(!is.na(notes), notes, ""),
      has_notes = !is.na(notes)
    ) |>
    ggplot(aes(x = datetime, y = value)) +
    # Zone bleue pour températures trop basses (< 36.5°C)
    annotate(
      "rect",
      xmin = min(temp_data$datetime),
      xmax = max(temp_data$datetime),
      ymin = -Inf,
      ymax = 36.5,
      fill = "blue",
      alpha = 0.1
    ) +
    # Zone rouge pour températures trop élevées (> 37.5°C)
    annotate(
      "rect",
      xmin = min(temp_data$datetime),
      xmax = max(temp_data$datetime),
      ymin = 37.5,
      ymax = Inf,
      fill = "red",
      alpha = 0.1
    ) +
    # Courbe de tendance
    geom_smooth(
      method = "loess",
      span = 0.3,
      color = "grey60",
      fill = "grey60",
      alpha = 0.2,
      linewidth = 1
    ) +
    # Points de mesure
    geom_point_interactive(
      aes(tooltip = tooltip, data_id = datetime, color = has_notes),
      fill = "grey40",
      size = 2.5,
      stroke = 1,
      shape = 21,
      hover_nearest = TRUE
    ) +
    scale_color_manual(
      values = c("TRUE" = "black", "FALSE" = "grey40"),
      guide = "none"
    ) +
    scale_x_datetime(date_labels = "%d %b") +
    labs(
      y = "Température (°C)",
      x = NULL
    ) +
    theme_minimal() +
    theme(
      panel.grid.minor = element_blank()
    )

  girafe(
    ggobj = plot_temp,
    options = list(opts_sizing(rescale = TRUE, width = .8))
  )
} else {
  cat("Aucune donnée de température à afficher.")
}
```


### Symptômes

```{r}
library(reactable)

symptomes_data <- data |>
  dplyr::filter(variable == "symptome") |>
  dplyr::arrange(dplyr::desc(datetime))

if (nrow(symptomes_data) > 0) {
  symptomes_data |>
    dplyr::select(datetime, notes) |>
    reactable(
      sortable = TRUE,
      columns = list(
        datetime = colDef(
          name = "Date et heure",
          format = colFormat(datetime = TRUE)
        ),
        notes = colDef(name = "Description")
      )
    )
} else {
  cat("Aucun symptôme enregistré.")
}
```
