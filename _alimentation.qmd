...
Au cours des 24 dernières heures, Édouard a bu **`r get_latest_daily_bottles(data)`** biberons, pour un total de **`r get_latest_daily_volume(data)`** ml de lait.
:::
...
ggplot(daily_volume, aes(x = date_only, y = total_volume)) +
  ...
  geom_text(
    aes(label = paste0(round(total_volume), " ml")),
    vjust = -0.5,
    size = 3,
    color = "gray30"
  ) +
